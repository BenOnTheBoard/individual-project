...
export class TextRendererService {
  ...
  readonly #startChar = 32;
  readonly #endChar = 256;
  readonly #supportedSpecialChars = '{}\n';

  #font = 'Arial';
  #charSizes: Array<number> = [];
  #fontSize: number;
  ...
  #renderTextSegment(text: string, state: TextRenderState): void {
    this.#ctx.save();
  ...
  #drawTextFromState(text: string, state: TextRenderState): void {
    if (!this.#ctx) throw new Error('Context not provided.');

    const xStart = state.pos.x;
    const colourStack: Array<string> = [];
    let subText = '';
    let width = 0;
  ...
  public drawText(text: string, pos: Position, colour: string = 'black'): void {
    ...
    const newRenderState = { colour: fontColour, pos };
